...
try:
    ...
    WINDOWS_AVAILABLE = False
    TOAST_AVAILABLE = False
    print("Toast notifications not available. Install with: pip install win10toast")
...
class TimerApp:
    def __init__(self, root):
        ...
        self.root.geometry("600x400")  # Reduced height since removing stats
        
        # Try to set up taskbar flashing (Windows-specific)
        self.setup_taskbar_flashing()

        self.timer_running = False
        self.timer_paused = False
        self.remaining_time = 0
        self.flash_active = False
        self.auto_restart = False  # New auto-restart mode
        self.timer_completed = False  # Track if timer naturally completed
        self.taskbar_flash_active = False  # Track taskbar flashing
        ...
        # Load usage statistics (keeping for potential future use)
        self.usage_stats = self._load_usage_stats()

        # ---------- CENTRAL COLUMN (everything centered) ----------
        # Vertical frame that holds: manual inputs, presets, Add Message button, Auto Restart button
        ...
        # Auto Restart button (NEW - centered BELOW presets)
        ...
        self.auto_restart_button.pack(pady=(8, 4))

        # Add Message button (centered BELOW auto restart)
        self.add_message_button = tk.Button(self.center_inner, text="Add Message", command=self._show_message_entry)
        self.add_message_button.pack(pady=(4, 0))
    ...
    def setup_taskbar_flashing(self):
        """Setup taskbar notifications (Windows-specific)"""
        self.notification_supported = TOAST_AVAILABLE
        if self.notification_supported:
            try:
                self.toaster = ToastNotifier()
                print("Toast notifications available")
            except Exception as e:
                print(f"Toast notification setup failed: {e}")
                self.notification_supported = False
        else:
            print("Toast notifications not available")

    def flash_taskbar_icon(self, start_flashing=True):
        """Show persistent toast notifications instead of flashing"""
        if not self.notification_supported:
            return
            
        if start_flashing:
            self.taskbar_flash_active = True
            threading.Thread(target=self._show_persistent_notifications, daemon=True).start()
            print("Started showing notifications")
        else:
            self.taskbar_flash_active = False
            print("Stopped showing notifications")

    def _show_persistent_notifications(self):
        """Show repeated toast notifications to grab attention"""
        try:
            notification_count = 0
            
            while self.taskbar_flash_active and notification_count < 20:  # Show up to 20 notifications
                try:
                    # Show toast notification
                    self.toaster.show_toast(
                        "⏰ Timer Finished!",
                        "Your focus session is complete. Click to return to the timer.",
                        icon_path=None,
                        duration=3,  # Show for 3 seconds
                        threaded=True  # Don't block
                    )
                    
                    print(f"Showed notification {notification_count + 1}")
                    notification_count += 1
                    
                    # Wait before next notification
                    time.sleep(4)  # Wait 4 seconds between notifications
                    
                except Exception as e:
                    print(f"Notification {notification_count} failed: {e}")
                    time.sleep(4)
                    notification_count += 1
    ...
    def toggle_auto_restart(self):
        ...
        if self.auto_restart:
            self.auto_restart_button.config(text="Auto Restart: ON", bg="#90EE90")  # Light green
        else:
            self.auto_restart_button.config(text="Auto Restart: OFF", bg="#f0f0f0")  # Default gray

    # ---------- Usage Statistics Methods (kept but not displayed) ----------
    ...
    def set_and_start_timer(self):
        try:
            ...
            self.timer_completed = False  # Reset completion flag
    ...
    def show_timer_screen(self, minutes, seconds):
        ...
        self.auto_restart_button.pack_forget()  # Hide auto restart button during timer
    ...
    def stop_timer(self):
        was_flashing = self.flash_active
        self.timer_running = False
        self.timer_paused = False
        self._stop_flashing_and_restore_bg()
        
        # FIXED: Check if timer completed AND auto-restart is on
        if self.auto_restart and self.timer_completed and was_flashing:
            print("Auto-restarting timer...")  # Debug output
            ...
            # Restart with the same duration
    ...
    def run_timer(self):
        ...
        if self.timer_running:
            self.timer_running = False
            self.timer_completed = True  # FIXED: Set completion flag
    ...
    def show_time_up(self):
        # Show time's up UI and start flashing (both app and notifications)
        ...
        self.flash_taskbar_icon(True)  # Start showing notifications

    # ---------- Enhanced flashing (app + notifications) ----------
    ...
    def _stop_flashing_and_restore_bg(self):
        if self.flash_active:
            self.flash_active = False
            self.flash_taskbar_icon(False)  # Stop notifications
        else:
    ...
    def sleep_for_5_minutes(self):
        ...
        self._stop_flashing_and_restore_bg()
        self.timer_completed = False  # Reset completion flag for sleep timer
        self.message_label.pack_forget()
        self.typing_prompt.pack_forget()
        self.typing_entry.pack_forget()
        self.sleep_button.pack_forget()
    ...
    def reset_ui(self):
        ...
        self.set_button.pack(pady=8)
        self.preset_frame.pack(pady=10)
        self.auto_restart_button.pack(pady=(8, 4))  # Show auto restart button again
        self.add_message_button.pack(pady=(4, 0))
